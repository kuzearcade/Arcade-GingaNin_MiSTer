...
# START_WAIT is 0x0C000000 cycles (~4.2 s at this core's 48 MHz clk_sys), not the ~65k the
...
# Per-set check-byte overrides (NMKBP964/NMK16 needed one); none for this board.
HS_CHECK_OVERRIDE = {}

# hiscore.dat records relative to a memory share would need its base here;
# ginganin's records are all `program` (CPU addresses in work RAM).
SHARE_BASE = {}
...
def records(lines,setname=None):
    ...
    for ln in lines:
        ...
        if len(f)<6: continue
        space=f[1].strip()
        if space not in ('program',) and space not in SHARE_BASE:
            raise SystemExit(f'{setname}: hiscore.dat space {space!r} is not mapped here')
        addr=int(f[2],16) + SHARE_BASE.get(space, 0); length=int(f[3],16)
...
def main():
    dat=load_dat(sys.argv[1] if len(sys.argv)>1 else os.path.expanduser('~/mame/plugins/hiscore/hiscore.dat'))
